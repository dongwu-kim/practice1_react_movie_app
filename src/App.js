      ...
      <div>
        {isLoading
          ...
          : movies.map((movie) => (
              ...
              movies Array의 각 obj들을 하나씩 가져오는 것이죠.
              사전에 API에서 sort_by=rating을 활용해 영화의 평점 순위에 따른 정렬을 완료했기 때문에
              우린 map() 메소드만으로도 편하게 component props value를 삽입할 수 있습니다. */
              <Movie
                key={movie.id}
                id={movie.id}
                title={movie.title}
              ...
              />
              // Movie의 props는 Movie.js에서 생성했습니다.
              // Movie의 value의 경우, movies 라는 Array의 object를 기반으로 구성되었습니다.
            ))}
      </div>
    );
  }
